...
const data = [
  {
    ...
    links: [
      ...
      { label: 'board', link: '/YonetimKurulu' },
  ...
  },
  {
    title: 'contact',
    links: [
      { label: 'phone', link: 'tel:(0274) 502 11 61' },
      { label: 'email', link: 'mailto:info@dumlupinarteknokent.com' },
    ],
  },
];
  ...
  const groups = data.map((group) => {
    const links = group.links.map((link, index) => (
      <Link key={index} href={link.link} style={{ textDecoration: 'none' }}>
        <Text className={classes.link}>{t(link.label)}</Text>
      </Link>
    ));
    return (
